...
export interface DocEditorModel {
...
export class DocEditorState {
  ...
  public async saveContent(
    ctx: StateContext<DocEditorModel>,
    action: DocEditorActions.SaveContent
  ) {
    try {
  ...
  public updateDocSessionContent(
    ctx: StateContext<DocEditorModel>,
    action: DocEditorActions.UpdateDocSessionContent
  ) {
    ctx.setState({
  ...
  public updateCaretPosition(
    ...
    const user = ctx
      .getState()
      .docSession.users?.find((el) => el.uuid === action.res.uuid);
  ...
  public updateUser(
    ...
    if (action.type === 'del') {
      newUsers =
        ctx
          .getState()
          .docSession?.users?.filter((el) => el.uuid !== action.user.uuid) ??
        [];
    }
